...
function iHeartYou(){
	...
	let pointingRight = String.fromCodePoint(0x1F449, 0x1F3FB);
	let pointingLeft = String.fromCodePoint(0x1F448, 0x1F3FB);
	let revolvingHearts = String.fromCodePoint(0x1F49E);

	if(window.location.hash){
		vtext.textContent = encryptString(decodeURIComponent(window.location.hash).substring(1));
	}else{
		vtext.innerHTML = `Daisuki-Desu<3<3<3<br>Watashi no koto suki desuka?!?!<br>${pointingRight}${pointingLeft}${revolvingHearts}`;
	}
